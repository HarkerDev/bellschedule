...
var ClockUpdateRate = setInterval(function(){startTime()},500);
...
//Shamelessly stolen from certain website
function startTime()
{
var today=new Date();
...
m=checkTime(m);
s=checkTime(s);
NH=h%12 //modding out by twelve to fix military time issue
document.getElementById('CurrentTime').innerHTML=NH+":"+m+":"+s;
//setInterval(function(){startTime()},500);
}

function checkTime(i) //Used to fix formatting, adds zeros before single digit times
{
if (i<10)
  {
  i="0" + i;
